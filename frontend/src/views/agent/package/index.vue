  ...
  <section class="package-manage">
    ...
    <section class="package-manage-body">
      <div class="package-manage-header">
        <div class="default_version" v-bk-tooltips="$t('通过修改稳定版本标签来指定')">
          <span class="title">{{ $t('默认升级安装版本') }}</span>：
          ...
          <div class="select-quick-head">
            ...
            <bk-compose-form-item style="display: flex; width: 100%;">
              ...
              <bk-select :value="dimension" style="flex: 1;" :clearable="false" @selected="updateOptionalList">
                <bk-option v-for="opt in dimensionList" :key="opt.id" :id="opt.id" :name="$t(opt.name)" />
              </bk-select>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDefaultVersion = async (type: PkgType = 'gse_agent') => {
