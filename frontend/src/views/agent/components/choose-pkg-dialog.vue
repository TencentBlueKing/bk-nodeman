...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const disabledClick = computed(() => {
      return !selectedVersion || (nochanges.value === props.versions.length && props.type === 'upgrade');
    });
    const handleConfirm = () => {
      emit('confirm', {
        osType: props.osType,
    ...
    const getCompareVersion = async (version: string) => {
      ...
      upgrades.value = upgrade_count || 0;
      rollbacks.value = downgrade_count || 0;
      nochanges.value = no_change_count || 0;
    }
