...
export default class PluginList extends Mixins(HeaderFilterMixins) {
  ...
  private tableLoading = true;  // 这里初始化设置为true, 防止页面刷新的时候出现table没有数据加载出来的情况
  ...
  public async handleBizSelect() {
    ...
    this.pagination.current = 1;
    const params = this.getCommonParams();
    // 此处不用有延迟的handleSearch方法，因为会被排在getPluginFilter后面执行
    this.tableLoading = true;
  ...
  private async created() {
    ...
    // params是用来搜索列表数据的参数，其中有个condition是搜索框的值，此处为created,所以condition为空[]
    ...
    await this.getStrategyTopo();
    await this.getFilterData();// 修复filter condition接口慢阻塞UI问题
    this.tableLoading = false;
    this.loading = false;
  }
  ...
  // 从搜索框的配置(getFilterList接口获取的数据)中获取插件状态映射
  private getPluginStatusMap() {
  ...
  public async getPluginFilter() {
    ...
    this.filterDate = deepClone(data);
  ...
  public getConditions(type: 'load' | 'operate' = 'load') {
    // getPluginStatusMap用于获取插件状态映射pluginStatusMap
    this.getPluginStatusMap();
