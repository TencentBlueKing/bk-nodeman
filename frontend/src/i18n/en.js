...
export default {
  ...
  删除: 'Delete',
  // 此处文案问题较大，暂时遗留。
  云区域管理提示一: 'A group of server units that can directly communicate with each other. such as a corporate LAN, the VPC (Virtual Private Network) of public cloud. Proxy: Proxy node for external communication of cloud area.',
  云区域管理提示二: 'In order to manage the host from different cloud area, it\'s neccessary to create at least one default proxy host. Blueking GSE is typically used to communicate with the external network can only pass, for other hosts in the area communication with Proxy.',
  变更云区域原有的接入点: 'To change the previous access point of the cloud area:',
  将会重装该云区域下所有Proxy: 'It will reinstall all of Proxy under the cloud area',
  阿里云: 'Ali Cloud',
  AWS: 'AWS',
  腾讯云: 'Tencent cloud',
  ...
  // （二确的文案，建议统一改成这种格式）
  确定删除该云区域: 'Are you sure to delete the cloud area?',
  业务范围: 'Business scope',
  自动选择非默认接入点: 'Automatically select non-default access point',
  自动选择接入点: 'Automatically select access point',
  一级云区域: 'L1 cloud area',
  下一步: 'Next',
  取消: 'Cancel',
  必填项: 'Required',
  ...
  请选择云区域: 'Please select cloud area',
  勾选IP: 'Checked IP',
  所有IP: 'All IP',
  异常IP: 'Abnormal IP',
  批量: 'Batch',
  ...
  业务: 'Business',
  云区域: 'Cloud area',
  // （待讨论,上下文有用area的，也有用region/zone的，需要统一。cloud area这个词容易引起误解，实际上只是一个隔离网络。需要上下文统一这个概念。）
  ...
  别名提示: 'To optimize the problem that the Cloud area name is too difficult to recognize, you can set a short alias for shown. Aliases can be edited, the cloud areas of different business may have the same alias.',
  ...
  提交: 'Submit',
  云区域创建成功: 'Cloud area created successfully',
  ...
  删除禁用提示: 'The cloud area has the host. (Proxy or P-Agent)',
  ...
  多业务可选云区域提示: 'When there are multiple businesses, Only cloud areas that are shared by the business can be selected',
  表格设置: 'Table setting',
  ...
  出口IP: 'WAN IP',
  Proxy未安装: 'Proxy is not installed in this cloud area, please select a cloud area again, or {0}',
  前往安装: 'Go install',
  Proxy过期: 'The Proxy authentication under this cloud area has expired, reselect cloud area, or {0}',
  前往更新: 'Go update',
  ...
  接入点已在云区域中设定: 'Access point has been set in the cloud area',
  全部业务: 'All Business',
  ...
  启用: 'Enable',
  停用: 'Disable',
  无法查看完整Agent信息: 'There are unauthorized businesses in this cloud area, so the Agent information cannot be viewed completely.',
  ...
  proxySetupHostIp: `Intranet IP address for communication with the Agent in this cloud area, ${$DHCP ? '{1}{2} cannot be empty at the same time' : 'Only supports IPv4'}.`,
  ...
  安装通道Desc: 'In a special complex network, where the target host cannot directly connect with the host in the cloud area, you can install the agent by specifying an "install channel". The default option is "Default Chnnel".',
  ...
  gseTopTips2: '2.GSE Server can deploy relay Servers according to the area; nearest access point may be selected when creating a cloud area',
  ...
  手动安装任务Tips: 'The installation task has been created, please click "Maual Install" in the list to continue',
  ...
  搜索IP云区域操作系统Agent状态: 'Search IP, cloud area, operating system, Agent status',
