    ...
    <bk-table
      ...
      <bk-table-column
        ...
        <template #default="{ row }">
          <bk-checkbox
            v-authority="{
              active: row.type === 'biz' && !row.has_permission,
              apply_info: getAuthInfo(row)
            }"
...
export default class IpSelectorTable extends Vue {
  ...
  public handleSelectionChange({ value, type }: { value: CheckValue, type: CheckType }) {
    ...
    this.checkType = type;
    this.excludeData = value === 0 ? [...this.data] : [...this.data.filter(item => item.disabled)];
    this.selections = value === 2
      ? [...this.data.filter(item => !item.disabled)]
      : [];
  ...
  private handleRowClass({ row }: { row: any }) {
    if (row.disabled) {
      return 'row-disabled';
    }
  }
