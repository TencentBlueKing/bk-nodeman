...
import { ITableConfig, ITableCheckData, SearchDataFuncType, IClassifyTab, IActive } from '../types/selector-type';
...
export default class CustomInput extends Vue {
  ...
  @Prop({ default: 'ip', type: String }) private readonly ipKey!: string;
  ...
  private async getTableData(params: any, type = '') {
    try {
      ...
      const data = defaultSearch(this.tabData[this.ipTab.active as IActive], params.tableKeyword || '')
      return {
        total: data.length,
        data,
      };
  ...
  private async handleParseDataChange(reqParams: any, type: string) {
    ...
    this.goodList.forEach((ip) => {
      // 对比返回值，找到全部错误IP
      !res.data.some(item => item[this.ipKey] === ip) && this.temErrList.push(ip);
    });
    ...
    this.ipTab.active = [
      'inner', 'outer', 'other',
    ].find(item => this.tabData[item as IActive] && this.tabData[item as IActive].length) as string || 'inner';
    setTimeout(() => {
      // 默认选择全部数据
