...
class AsymmetricHandler:
    ...
    def get_or_generate_key_pair_in_db(
    ...
    ) -> Dict[str, Union[models.AsymmetricKey, BaseAsymmetricCipher]]:
        """
        获取或生成非对称加密的私钥/密钥到DB
        :param name: 密钥名称
        ...
        try:
            private_key = models.AsymmetricKey.objects.get(
                name=name,
            ...
            public_key, __ = models.AsymmetricKey.objects.update_or_create(
                name=name,
        ...
        except models.AsymmetricKey.DoesNotExist:
            # 私钥不存在的情况下，重新生成
            cipher: BaseAsymmetricCipher = get_asymmetric_cipher()
            # 公私钥的更新需保证原子性
            with transaction.atomic():
                private_key = models.AsymmetricKey.objects.create(
                    name=name,
                    type=constants.AsymmetricKeyType.PRIVATE_KEY.value,
                    cipher_type=crypto_settings.ASYMMETRIC_CIPHER_TYPE,
    ...
    def fetch_public_keys(cls, names: List[str]) -> List[Dict[str, Any]]:
        ...
        :return:
        """
        public_key_details: List[Dict[str, Any]] = models.AsymmetricKey.objects.filter(
            type=constants.AsymmetricKeyType.PUBLIC_KEY.value,
            name__in=set(names),
            cipher_type=crypto_settings.ASYMMETRIC_CIPHER_TYPE,
        ...
        for name in internal_key_names_to_be_created:
            ...
            public_key: models.AsymmetricKey = cls.get_or_generate_key_pair_in_db(name=name)["public_key"]
