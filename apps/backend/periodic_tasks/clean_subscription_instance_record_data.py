...
def clean_subscription_instance_record_data():
    ...
    with connection.cursor() as cursor:
        ...
        if not need_clean_task_ids:
            logger.info(
                "need clean instance records_ids is empty, "
        ...
        if need_clean_pipeline_tree_ids:
            ...
            delete_data(
                cursor,
                "node_man_pipelinetree",
                delete_pipeline_tree_sql,
                need_clean_pipeline_tree_ids
            )
...
def get_need_clean_task_ids(cursor, policy_subscription_ids: list, alive_days, limit):
    ...
    if policy_subscription_ids:
        select_clean_records_sql: str = f"SELECT DISTINCT task_id FROM node_man_subscriptioninstancerecord " \
...
def generate_delete_sql(table_name: str, scope: Dict[str, Union[str, bool, list]], time_cond: int = None,
                        additional_cond: dict = None, limit: int = None):
    """生成删除语句"""
