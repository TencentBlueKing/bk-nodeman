...
from dataclasses import _FIELD, _FIELD_BASE, dataclass
from typing import (
    Any,
    Callable,
    Dict,
    ...
    def __dataclass_fields__(self):
        ...
        for cls in self.__class__.__mro__:
            for attr_name, attr_value in cls.__dict__.items():
                # 检查属性值是否是一个 property 对象
                if isinstance(attr_value, property) and not attr_name == "__dataclass_fields__":
                    # 如果是，则将其名称添加到列表中
                    data[attr_name] = DataClassFields(name=attr_name, _field_type=_FIELD)
        return data
