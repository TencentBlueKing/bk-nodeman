...
class InstallService(AgentBaseService):
    ...
    def handle_lan_windows_sub_inst(self, install_sub_inst_objs: List[InstallSubInstObj]):
        ...
        for install_sub_inst_obj in install_sub_inst_objs:
            ...
            push_curl_exe_params_list.append(
                {"sub_inst_id": install_sub_inst_obj.sub_inst_id, "host": host, "dest_dir": dest_dir}
            )
    ...
    @batch_call_single_exception_handler
    def execute_windows_commands(self, sub_inst_id, host, commands):
    ...
    @batch_call_single_exception_handler
    def push_curl_exe(self, sub_inst_id, host, dest_dir):
    ...
    def handle_report_data(self, sub_inst_id: int, success_callback_step: str) -> Dict:
        ...
        report_data.reverse()
        order_data = [json.loads(data.decode()) for data in report_data]
        cpu_arch = None
        is_finished = False
        error_log = ""
        logs = []
