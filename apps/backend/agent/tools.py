...
from apps.node_man.constants import SetupScriptFileName
from apps.node_man.models import aes_cipher
from apps.utils.basic import suffix_slash


class InstallationTools:
    def __init__(self, script_file_name, dest_dir, win_commands, upstream_nodes, jump_server, pre_commands, run_cmd):
...
def fetch_gse_servers(host: models.Host) -> Tuple:
    ...
    if host.install_channel_id:
        ...
        package_url = f"http://{jump_server.inner_ip}:17980/"
...
def choose_script_file(host: models.Host):
    """选择脚本文件"""
    ...
    if host.install_channel_id or (host.node_type == constants.NodeType.PAGENT and not host.is_manual):
        ...
        return SetupScriptFileName.SETUP_PAGENT_PY.value

    # 其它场景，按操作系统来区分
    script_file_name_map = {
        constants.OsType.LINUX: SetupScriptFileName.SETUP_AGENT_SH.value,
        constants.OsType.WINDOWS: SetupScriptFileName.SETUP_AGENT_BAT.value,
        constants.OsType.AIX: SetupScriptFileName.SETUP_AGENT_KSH.value,
    }
    script_file_name = script_file_name_map[host.os_type]
    return script_file_name


def format_run_cmd_by_os_type(os_type: str, run_cmd: str):
...
def gen_commands(host, pipeline_id, is_uninstall):
    """
    生成安装命令
    :param host: 主机信息
    ...
    if script_file_name == SetupScriptFileName.SETUP_PAGENT_PY.value:
        ...
        run_cmd_params.extend(
            [
                ...
                f"-o http://{jump_server.inner_ip}:17980/",
        ...
        # WINDOWS 下的 Agent 安装
        win_remove_cmd = f"del /q /s /f {dest_dir}{script_file_name} {dest_dir}{SetupScriptFileName.GSECTL_BAT.value}"
        win_download_cmd = (
            f"{dest_dir}curl.exe {host.ap.package_inner_url}/{script_file_name}"
            f" -o {dest_dir}{script_file_name} -sSf"
            f" && "
            f"{dest_dir}curl.exe {host.ap.package_inner_url}/{SetupScriptFileName.GSECTL_BAT.value}"
            f" -o {dest_dir}{SetupScriptFileName.GSECTL_BAT.value} -sSf"
        )
