...
class GseClientPack(object):
    def __init__(self, cert_package: str, client_package: str, server_package=str):
    ...
    def pack_gse_client(
        ...
        pkg_name_version = constants.AgentPackageMap.GSE_AGENT_PACKAGE_RE.match(
            os.path.basename(self.client_package)
        ).groupdict()["version"]

        self.extra_file(self.cert_package)
        self.extra_file(self.client_package)
        ...
        self.pack_agent_client(cpu_types=constants.CPU_TUPLE, yaml_config=yaml_config, os_types=constants.OS_TUPLE)
    ...
    def compression_script_file(
        self, target_path, os_type=constants.OsType.LINUX, client_type=constants.NodeType.AGENT
    ):
        pass

    def pack_agent_client(self, cpu_types: List[str], yaml_config: Dict[str, str], os_types: List[str]):
    ...
    def compression_package(
        ...
        record = {
            ...
            "cert_name": self.cert_package,
            "client_name": self.client_package,
            "version": version,
