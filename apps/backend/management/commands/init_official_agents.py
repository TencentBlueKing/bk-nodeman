...
from apps.backend.exceptions import CertFileNotFoundError, GsePackageNotFound
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        cert_file_package = self.filter_cert_package()
        ...
        similar_gse_agent_packages: List[str] = self.filter_target_files(
            regex=constants.AgentPackageMap.GSE_AGENT_PACKAGE_RE, file_path=settings.BK_OFFICIAL_AGENT_INIT_PATH
        )
        similar_gse_server_packages: List[str] = self.filter_target_files(
            regex=constants.AgentPackageMap.GSE_AGENT_PACKAGE_RE, file_path=settings.BK_OFFICIAL_AGENT_INIT_PATH
        )

        gse_agent_package = self.filter_last_package(file_list=similar_gse_agent_packages)
        gse_server_package = self.filter_last_package(file_list=similar_gse_server_packages)

        GseClientPack(
            cert_package=cert_file_package, client_package=gse_agent_package, server_package=gse_server_package
        ).pack_gse_client()
    ...
    def filter_cert_package(cls):
        ...
        if cert_package:
            return cert_package
        # 初始化路径下的包到配置文件
        similar_cert_files = cls.filter_target_files(
    ...
    @classmethod
    def filter_last_package(cls, file_list: List[str]) -> str:
        """
        选取目标文件中最新的一个
        """
    ...
    @classmethod
    def filter_target_files(cls, regex: Pattern[str], file_path: str) -> List[str]:
        """
        选出目标目录下所有符合预期正则表达式的文件
        """
        similar_file_list: List[str] = []
        __, files = cls.storage.listdir(file_path)
        for file_name in files:
            file_path = os.path.join(file_path, file_name)
            if regex.match(file_name):
                similar_file_list.append(file_path)
        if not similar_file_list:
            raise GsePackageNotFound(context="未匹配正确的Agent包文件")
