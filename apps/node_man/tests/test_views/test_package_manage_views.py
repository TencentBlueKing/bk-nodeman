...
class PackageManageViewsTestCaseUsingProxy(Proxy2StepAdapterTestCase):
    ...
    def test_list_with_filter_condition(self, *args, **kwargs):
        ...
        result = self.client.get(
            path="/api/agent/package/", data={"page": 1, "pagesize": 2, "tag_names": "tag1", "project": "gse_proxy"}
        )
        self.assertEqual(len(result["data"]["list"]), 1)
        self.assertIn("tag1", self.collect_all_tag_names(result["data"]["list"][0]["tags"]))
        result = self.client.get(
            path="/api/agent/package/", data={"page": 1, "pagesize": 2, "tag_names": "tag2", "project": "gse_proxy"}
        )
        self.assertEqual(len(result["data"]["list"]), 1)
        self.assertIn("tag2", self.collect_all_tag_names(result["data"]["list"][0]["tags"]))
        result = self.client.get(
            path="/api/agent/package/", data={"page": 1, "pagesize": 2, "tag_names": "tag3", "project": "gse_proxy"}
        )
