...
from apps.mock_data.common_unit.host import AP_MODEL_DATA, DEFAULT_IP, DEFAULT_IPV6
...
class TestApTransform(TestCase):
    ...
    def setUp(self):
        test_data_list = [
            ...
            },
            {
                ...
                "zk_hosts": [{"zk_ip": "127.0.0.1", "zk_port": "2182"}],
                "zk_account": "noneed",
                "zk_password": "noneed",
    ...
    def test_ap_transform(self):
        ...
        self.assertEqual(
            ...
            {
                "inner_ips": [{"inner_ip": "127.0.0.198"}],
                "outer_ips": [{"outer_ip": "127.0.0.69"}, {"outer_ip": "127.0.0.76"}],
            },
        )
        ...
        self.assertEqual(
            ...
            {
                "inner_ips": [{"inner_ip": "127.0.0.120"}, {"inner_ip": "127.0.0.27"}],
                "outer_ips": [{"outer_ip": "127.0.0.20"}, {"outer_ip": "127.0.0.27"}],
            },
        )
    ...
    def test_transfrom_command(self):
        ...
        call_command("transform_ap_data", transform_endpoint_to_leagcy=True, transform_ap_id=default_ap_id)
        ...
        self.assertEqual(
            ...
            {
                "inner_ips": [{"inner_ip": "", "inner_ipv6": DEFAULT_IPV6}],
                "outer_ips": [{"outer_ip": "", "outer_ipv6": self.mock_outer_ipv6(DEFAULT_IPV6)}],
            },
        )

        # 转换回旧的数据
        call_command("transform_ap_data", transform_endpoint_to_leagcy=True, transform_ap_id=gse_v1_ap.id)
        self.assertEqual(
        ...
        # 校验参数
        self.assertRaises(
            ValueError, call_command, "transform_ap_data", transform_endpoint_to_leagcy=True, transform=True
        )
        self.assertRaises(
            ValueError, call_command, "transform_ap_data", transform_endpoint_to_leagcy=True, transform=False
        )
        self.assertRaises(
            ValueError,
            call_command,
            "transform_ap_data",
            transform_endpoint_to_leagcy=True,
            transform=False,
            all_ap=True,
...
class ApViewTransformTestCase(CustomAPITestCase):
    ...
    def test_mix_ip_ap_create(self):
        self.ap_data["taskserver"] = {"inner_ips": [{"inner_ip": DEFAULT_IP}, {"inner_ipv6": DEFAULT_IPV6}]}
        result = self.client.post(self.CREATE_URL, self.ap_data)
        self.assertFalse(result["result"])
        self.assertEqual(result["message"], "inner_ips 中不能同时包括 inner_ip 和 inner_ipv6（3800001）")

        # 支持 taskserver v6 & fileserver v4 混合
        mix_server_ap_data = deepcopy(self.ap_data)
        mix_server_ap_data["taskserver"] = {"inner_ips": [{"inner_ipv6": DEFAULT_IPV6}]}
    ...
    def test_multi_v4_ap_create(self):
        ...
        self.ap_data["taskserver"] = {
            "inner_ips": [{"inner_ip": DEFAULT_IP}, {"inner_ip": DEFAULT_IP.replace("1", "2")}],
            "outer_ips": [{"outer_ip": DEFAULT_IP}, {"outer_ip": DEFAULT_IP.replace("1", "2")}],
        }
        result = self.client.post(self.CREATE_URL, self.ap_data)
        self.assertTrue(result["result"])
        ap = models.AccessPoint.objects.get(name=self.TEST_AP_NAME)
    ...
    def test_multi_v6_ap_create(self):
        # 支持多个 v6 地址
        self.ap_data["taskserver"] = {
            "inner_ips": [{"inner_ipv6": DEFAULT_IPV6}, {"inner_ipv6": DEFAULT_IPV6.replace("1", "2")}]
        }
        result = self.client.post(self.CREATE_URL, self.ap_data)
        self.assertTrue(result["result"])
    ...
    def test_v4_v6_mix_ap_create(self):
        ...
        self.ap_data["taskserver"] = {
            ...
            "outer_ips": [{"outer_ipv6": DEFAULT_IPV6}],
        }
        result = self.client.post(self.CREATE_URL, self.ap_data)
        self.assertTrue(result["result"])
        ap = models.AccessPoint.objects.get(name=self.TEST_AP_NAME)
    ...
    def test_filter_ip_ap_crete(self):
        # 支持过滤掉重复 ip
        self.ap_data["taskserver"] = {"inner_ips": [{"inner_ip": DEFAULT_IP}, {"inner_ip": DEFAULT_IP}]}
        result = self.client.post(self.CREATE_URL, self.ap_data)
        self.assertTrue(result["result"])
        self.assertEqual(
            result["data"]["taskserver"],
            {
                "inner_ips": [{"inner_ip": DEFAULT_IP}],
            },
        )
    ...
    def test_update_ap(self):
        ...
        self.ap_data["taskserver"] = {"inner_ips": [{"inner_ip": "111.1.1.1"}]}
        ...
        self.assertEqual(ap_obj.taskserver, {"inner_ips": [{"inner_ip": "111.1.1.1"}]})
