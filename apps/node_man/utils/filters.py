...
class GsePackageFilter(FilterSet):
    ...
    created_time = django_filters.CharFilter(field_name="created_time", method="filter_created_time")
    ...
    def filter_created_time(self, queryset, name, value):
        return queryset.filter(created_time__gte=datetime.strptime(value, "%Y-%m-%d-%H:%M:%S"))

    @classmethod
    def str_to_list(cls, s, excepted_type="str"):
        conditions = [tag_name.replace("&nbsp;", "").strip() for tag_name in s.split(",")]
        if excepted_type == "bool":
            conditions = [True if c.lower() == "true" else False for c in conditions]
        return conditions
