...
class PolicyHandler:
    @staticmethod
    def get_plugin_desc(plugin_name: str) -> models.GsePluginDesc:
        """Get plugin desc with official plugin compatibility."""
        return models.GsePluginDesc.objects.get(
    ...
    def policy_info(policy_id: int) -> Dict[str, Any]:
        ...
        plugin_desc_obj = PolicyHandler.get_plugin_desc(policy_info["plugin_name"])
        policy_info.update(
            {
                "plugin_info": {
    ...
    def selected_preview(query_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Official plugins are shared globally, while third-party plugins are isolated by tenant.
        plugin_obj: models.GsePluginDesc = PolicyHandler.get_plugin_desc(query_params["steps"][0]["id"])
