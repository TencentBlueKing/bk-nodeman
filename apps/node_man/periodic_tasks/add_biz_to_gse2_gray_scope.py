...
def sync_new_biz_to_gray_scope_list():
    ...
    if new_biz_ids:
        with transaction.atomic():
            ...
            GlobalSettings.update_config(key=GlobalSettings.KeyEnum.ALL_BIZ_IDS.value, value=all_biz_ids)
            # 将新业务ID加入灰度列表
            gray_scope_list: List[int] = GlobalSettings.get_config(
                key=GlobalSettings.KeyEnum.GSE2_GRAY_SCOPE_LIST.value, default=[]
            )
            gray_scope_list.extend(new_biz_ids)
            GlobalSettings.update_config(key=GlobalSettings.KeyEnum.GSE2_GRAY_SCOPE_LIST.value, value=gray_scope_list)
