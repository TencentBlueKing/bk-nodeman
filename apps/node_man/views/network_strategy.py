...
class NetworkStrategyViews(APIViewSet):
    @swagger_auto_schema(
        operation_summary="云区域安装GSE策略",
        tags=NETWORK_STRATEGY_VIEW_TAGS,
    )
    @action(detail=False, methods=["POST"], serializer_class=network_strategy.InstallStrategySerializer)
    def install_strategy(self, request):
        """
        @api {POST} /network_strategy/install_agent_strategy/  云区域安装Agent、proxy策略
        @apiName install_agent_strategy
        @apiGroup NetworkStrategy
        @apiParam {List} host_info 安装主机信息
        ...
        [
            {
                "agent_strategy_name": "test-默认接入点1"
                "agent_strategy_data": [
                    {
            ...
            {
                "proxy_strategy_name": "test-默认接入点2"
                "proxy_strategy_data": [
                    {
                        "source_address": "127.0.0.1",
                        "target_address": "127.0.0.2"
        ...
        return Response(NetworkStrategyHandler().install_strategy(validated_data["host_info"]))
