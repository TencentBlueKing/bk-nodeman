...
TEST_LOGS=$(coverage run --include "$COVERAGE_INCLUDE_MODULES" --omit "$COVERAGE_OMIT_PATH" ./manage.py test apps.core apps.node_man apps.backend apps.utils apps.iam 2>&1)
echo "${TEST_LOGS}"
