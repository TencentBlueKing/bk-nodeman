...
TEST_MODULES="apps.node_man apps.backend apps.core apps.utils"

# 删除coverage历史归档文件
coverage erase

TEST_LOGS=$(coverage run --include "$COVERAGE_INCLUDE_MODULES" --omit "$COVERAGE_OMIT_PATH" ./manage.py test "$TEST_MODULES" 2>&1)
echo "${TEST_LOGS}"
