...
from impacket.dcerpc.v5.rpcrt import DCERPC_v4, DCERPC_v5, DCERPCException
...
class HTTPTransport(TCPTransport, RPCProxyClient):
    ...
    def set_credentials(self, username, password, domain="", lmhash="", nthash="", aesKey="", TGT=None, TGS=None):
        return self._transport.set_credentials(self, username, password, domain, lmhash, nthash, aesKey, TGT, TGS)
        ...
        if not self.__smb_connection:
            ...
            if self._strict_hostname_validation:
                self.__smb_connection.setHostnameValidation(
                    self._strict_hostname_validation, self._validation_allow_absent, self._accepted_hostname
                )
