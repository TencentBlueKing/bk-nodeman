...
from impacket import LOG, nmb, nt_errors, smb, smb3
...
class SMBConnection:
    ...
    def __init__(
        ...
        remoteHost="",
        myName=None,
        sess_port=nmb.SMB_SESSION_PORT,
        timeout=60,
    ...
    def negotiateSession(
        ...
        if preferredDialect is None:
            ...
            if preferredDialect == smb.SMB_DIALECT:
                self._SMBConnection = smb.SMB(
                    self._remoteName, self._remoteHost, self._myName, hostType, self._sess_port, self._timeout
                )
    ...
    def negotiateSessionWildcard(
        self, myName, remoteName, remoteHost, sess_port, timeout, extended_security=True, flags1=0, flags2=0, data=None
    ):
        ...
        resp = None
        while tries < 2:
            self._nmbSession = nmb.NetBIOSTCPSession(
                myName, remoteName, remoteHost, nmb.TYPE_SERVER, sess_port, timeout
            )
            ...
            try:
                if self.getDialect() == smb.SMB_DIALECT:
                    return self._SMBConnection.kerberos_login(
                        user, password, domain, lmhash, nthash, aesKey, kdcHost, TGT, TGS
                    )
    ...
    def listShares(self):
        ...
        rpctransport = transport.SMBTransport(
            self.getRemoteName(), self.getRemoteHost(), filename=r"\srvsvc", smb_connection=self
        )
    ...
    def createFile(
        ...
        if self.getDialect() == smb.SMB_DIALECT:
            ...
            try:
                return self._SMBConnection.create(
                    treeId,
                    pathName,
                    desiredAccess,
                    shareMode,
    ...
    def writeNamedPipe(self, treeId, fileId, data, waitAnswer=True):
        ...
        try:
            if self.getDialect() == smb.SMB_DIALECT:
                return self._SMBConnection.write_andx(
                    treeId, fileId, data, wait_answer=waitAnswer, write_pipe_mode=True
                )
            else:
    ...
    def listSnapshots(self, tid, path):
        ...
        fid = self.openFile(
            ...
            fileAttributes=None,
            creationOption=FILE_SYNCHRONOUS_IO_NONALERT,
            shareMode=FILE_SHARE_READ | FILE_SHARE_WRITE,
        )

        # first send with maxOutputResponse=16 to get the required size
        try:
            snapshotData = SRV_SNAPSHOT_ARRAY(
                self._SMBConnection.ioctl(
                    tid, fid, FSCTL_SRV_ENUMERATE_SNAPSHOTS, flags=SMB2_0_IOCTL_IS_FSCTL, maxOutputResponse=16
                )
            )
        except (smb.SessionError, smb3.SessionError) as e:
            ...
            try:
                snapshotData = SRV_SNAPSHOT_ARRAY(
                    self._SMBConnection.ioctl(
                        ...
                        maxOutputResponse=snapshotData["SnapShotArraySize"] + 12,
                    )
                )
            except (smb.SessionError, smb3.SessionError) as e:
                self.closeFile(tid, fid)
    ...
    def removeMountPoint(self, tid, path):
        ...
        try:
            self._SMBConnection.ioctl(
                tid, fid, FSCTL_DELETE_REPARSE_POINT, flags=SMB2_0_IOCTL_IS_FSCTL, inputBlob=reparseData
            )
        except (smb.SessionError, smb3.SessionError) as e:
            self.closeFile(tid, fid)
            raise SessionError(e.get_error_code(), e.get_error_packet())
    ...
    def reconnect(self):
        ...
        if self._doKerberos is True:
            self.kerberosLogin(
                userName, password, domain, lmhash, nthash, aesKey, self._kdcHost, TGT, TGS, self._useCache
            )
        else:
