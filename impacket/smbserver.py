    ...
    try:
        if len(ntresponse) > 24:
            ...
            ret_value = {
                ...
                % (
                    username.decode("utf-16le"),
                    domain.decode("utf-16le"),
                    hexlify(challenge).decode("latin-1"),
                    hexlify(ntresponse).decode("latin-1")[:32],
                    hexlify(ntresponse).decode()[32:],
                ),
                "hash_version": "ntlmv2",
            }
        else:
            ...
            ret_value = {
                ...
                % (
                    username.decode("utf-16le"),
                    domain.decode("utf-16le"),
                    hexlify(lmresponse).decode("latin-1"),
        ...
        try:
            if len(ntresponse) > 24:
                ...
                ret_value = {
                    "hash_string": "%s::%s:%s:%s:%s"
                    % (
                        username.decode("latin-1"),
                        domain.decode("latin-1"),
                        hexlify(challenge).decode("latin-1"),
                        hexlify(ntresponse)[:32].decode("latin-1"),
                        hexlify(ntresponse)[32:].decode("latin-1"),
                    ),
                    "hash_version": "ntlmv2",
                }
...
def findFirst2(path, fileName, level, searchAttributes, pktFlags=smb.SMB.FLAGS2_UNICODE, isSMB2=False):
    ...
    for i in files:
        ...
        if (
            level == smb.SMB_FIND_FILE_BOTH_DIRECTORY_INFO
            or level == smb.SMB_FIND_FILE_ID_BOTH_DIRECTORY_INFO
            or level == smb2.SMB2_FILE_ID_BOTH_DIRECTORY_INFO
            or level == smb2.SMB2_FILE_BOTH_DIRECTORY_INFO
        ):
            item["EaSize"] = 0
            item["EndOfFile"] = size
            ...
            level == smb.SMB_FIND_FILE_FULL_DIRECTORY_INFO
            or level == smb.SMB_FIND_FILE_ID_FULL_DIRECTORY_INFO
            or level == smb2.SMB2_FULL_DIRECTORY_INFO
        ):
            item["EaSize"] = 0
            item["EndOfFile"] = size
            item["AllocationSize"] = size
...
class SMBCommands:
    ...
    def smbTransaction(connId, smbServer, SMBCommand, recvPacket, transCommands):
        ...
        if transParameters["ParameterCount"] != transParameters["TotalParameterCount"]:
            ...
            if command in transCommands:
                ...
                data = b""
                try:
                    setup, parameters, data, errorCode = transCommands[command](
                        connId,
                        smbServer,
                        recvPacket,
    ...
    def smbQueryInformationDisk(connId, smbServer, SMBCommand, recvPacket):
        ...
        # Get the Tid associated
        if recvPacket["Tid"] in connData["ConnectedShares"]:
            totalUnits, freeUnits = queryDiskInformation(connData["ConnectedShares"][recvPacket["Tid"]]["path"])
    ...
    def smbComNtCreateAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if recvPacket["Tid"] in connData["ConnectedShares"]:
            ...
            if createDisposition == smb.FILE_SUPERSEDE:
                ...
                if (
                    os.path.exists(pathName) is not True
                    and (str(pathName) in smbServer.getRegisteredNamedPipes()) is not True
                ):
                    errorCode = STATUS_NO_SUCH_FILE
    ...
    def smbComOpenAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if recvPacket["Tid"] in connData["ConnectedShares"]:
            path = connData["ConnectedShares"][recvPacket["Tid"]]["path"]
            openedFile, mode, pathName, errorCode = openFile(
                path,
        ...
        if SMBCommand["WordCount"] == 12:
            ...
            if messageType == 0x01:
                ...
                if len(smbServer.getCredentials()) > 0:
                    ...
                    if identity in smbServer.getCredentials():
                        ...
                        errorCode, sessionKey = computeNTLMv2(
                            identity,
                            lmhash,
                            nthash,
                            smbServer.getSMBChallenge(),
                            authenticateMessage,
                            connData["CHALLENGE_MESSAGE"],
                ...
                if errorCode == STATUS_SUCCESS:
                    ...
                    smbServer.log(
                        "User %s\\%s authenticated successfully"
                        % (
                            authenticateMessage["host_name"].decode("utf-16le"),
                            authenticateMessage["user_name"].decode("utf-16le"),
                        )
                    ...
                    try:
                        ...
                        if jtr_dump_path != "":
                            writeJohnOutputToFile(
                                ntlm_hash_data["hash_string"], ntlm_hash_data["hash_version"], jtr_dump_path
                            )
                    except:
            ...
            try:
                ...
                ntlm_hash_data = outputToJohnFormat(
                    ...
                    b(sessionSetupData["PrimaryDomain"]),
...
class SMB2Commands:
        ...
        if struct.unpack("B", securityBlob[0:1])[0] == ASN1_AID:
            ...
            if len(blob["MechTypes"][0]) > 0:
                ...
                if mechType != TypesMech["NTLMSSP - Microsoft NTLM Security Support Provider"]:
                    ...
                    # ToDo: Build this into a SPNEGO_NegTokenResp()
                    respToken = (
                        b"\xa1\x15\x30\x13\xa0\x03\x0a\x01\x03\xa1\x0c\x06\x0a\x2b\x06\x01\x04\x01\x82\x37\x02\x02\x0a"
                    )
        ...
        if fileID in connData["OpenedFiles"]:
            ...
            try:
                if fileHandle == PIPE_FILE_DESCRIPTOR:
                    ...
                    os.close(fileHandle)
                    infoRecord, errorCode = queryFileInformation(
                        os.path.dirname(pathName), os.path.basename(pathName), smb2.SMB2_FILE_NETWORK_OPEN_INFO
                    )
            except Exception as e:
            ...
            if fileID in connData["OpenedFiles"]:
                ...
                if queryInfo["InfoType"] == smb2.SMB2_0_INFO_FILE:
                    if queryInfo["FileInfoClass"] == smb2.SMB2_FILE_INTERNAL_INFO:
                        ...
                        infoRecord, errorCode = queryFileInformation(
                            os.path.dirname(fileName), os.path.basename(fileName), queryInfo["FileInfoClass"]
                        )
                elif queryInfo["InfoType"] == smb2.SMB2_0_INFO_FILESYSTEM:
                    if queryInfo["FileInfoClass"] == smb2.SMB2_FILE_EA_INFO:
                        infoRecord = b"\x00" * 4
                    else:
                        infoRecord = queryFsInformation(
                            os.path.dirname(fileName), os.path.basename(fileName), queryInfo["FileInfoClass"]
                        )
    ...
    def smb2QueryDirectory(connId, smbServer, recvPacket):
        ...
        if (
            connData["OpenedFiles"][fileID]["Open"]["EnumerationLocation"] == 0
            and connData["OpenedFiles"][fileID]["Open"]["EnumerationSearchPattern"] == ""
        ):
            if pattern == "":
                pattern = "*"
        ...
        if queryDirectoryRequest["Flags"] & smb2.SMB2_INDEX_SPECIFIED and queryDirectoryRequest["FileNameLength"] > 0:
            connData["OpenedFiles"][fileID]["Open"]["EnumerationSearchPattern"] = pattern
        ...
        while True:
            try:
                ...
                session = nmb.NetBIOSTCPSession(
                    self.__SMB.getServerName(),
                    "HOST",
                    self.__ip,
                    sess_port=self.__port,
                    sock=self.__request,
                    select_poll=self.__select_poll,
                )
                try:
                ...
                if connData["Authenticated"] is False and packet["Command"] not in (
                    ...
                    if packet["Command"] == smb.SMB.SMB_COM_TRANSACTION2:
                        respCommands, respPackets, errorCode = self.__smbCommands[packet["Command"]](
                            connId, self, SMBCommand, packet, self.__smbTrans2Commands
                        )
                    elif packet["Command"] == smb.SMB.SMB_COM_NT_TRANSACT:
                        respCommands, respPackets, errorCode = self.__smbCommands[packet["Command"]](
                            connId, self, SMBCommand, packet, self.__smbNTTransCommands
                        )
                    elif packet["Command"] == smb.SMB.SMB_COM_TRANSACTION:
                        respCommands, respPackets, errorCode = self.__smbCommands[packet["Command"]](
                            connId, self, SMBCommand, packet, self.__smbTransCommands
                        )
                    else:
                        if packet["Command"] in self.__smbCommands:
                            ...
                            respCommands, respPackets, errorCode = self.__smbCommands[255](
                                connId, self, SMBCommand, packet
                            )
    ...
    def __init__(self, listenAddress="0.0.0.0", listenPort=445, configFile=""):
        if configFile != "":
            ...
            self.__smbConfig.set(
                "global", "server_name", "".join([random.choice(string.ascii_letters) for _ in range(8)])
            )
            self.__smbConfig.set(
                "global", "server_os", "".join([random.choice(string.ascii_letters) for _ in range(8)])
            )
            self.__smbConfig.set(
                "global", "server_domain", "".join([random.choice(string.ascii_letters) for _ in range(8)])
            )
