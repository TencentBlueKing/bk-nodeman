...
class HTTPClientSecurityProvider:
    ...
    def get_auth_headers_basic(self, http_obj, method, path, headers):
        if (
            ...
            raise Exception(
                "Basic authentication in HTTP connection used, " "so set a plaintext credentials to connect."
            )
    ...
    def send_ntlm_type1(self, http_obj, method, path, headers, negotiateMessage):
        ...
        if res.status != 401:
            raise Exception(
                "Status code returned: %d. " "Authentication does not seem required for url %s" % (res.status, path)
            )

        if res.getheader("WWW-Authenticate") is None:
            raise Exception("No authentication requested by " "the server for url %s" % path)
    ...
    def get_auth_headers_auto(self, http_obj, method, path, headers):
        if self.__aesKey != "" or self.__TGT != None or self.__TGS != None:
            raise Exception("NTLM authentication in HTTP connection used, " "cannot use Kerberos.")
        ...
        if serverChallenge is not None:
            ...
            type3, exportedSessionKey = ntlm.getNTLMSSPType3(
                auth, serverChallenge, self.__username, self.__password, self.__domain, self.__lmhash, self.__nthash
            )
