...
get_config () {
    ...
    local config=(${AGENT_CONFIG_FILE})
...
download_pkg () {
    ...
    curl --connect-timeout 5 -o "$TMP_DIR/$PKG_NAME" \
            --progress-bar -w "%{http_code}" "$COMPLETE_DOWNLOAD_URL" >"$tmp_stdout" 2>"$tmp_stderr" &
    curl_pid=$!
    ...
    if [[ $http_status != "200" ]] && [[ "$http_status" != "000" ]]; then
        fail download_pkg FAILED "file $PKG_NAME download failed. (url:$COMPLETE_DOWNLOAD_URL, http_status:$http_status)"
    fi
...
COMPLETE_DOWNLOAD_URL="${DOWNLOAD_URL}"/agent/solaris/sparc/"${PKG_NAME}"
