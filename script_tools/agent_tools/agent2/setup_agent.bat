...
:is_process_start_ok
    ...
    if %errorlevel% neq 0 (
        call :print FAILED setup_agent DOME "Process gse_agent_daemon.exe start failed"
        call :multi_report_step_status
        exit /b 2
    ) else (
    ...
    if %errorlevel% neq 0 (
        call :print FAILED setup_agent DONE "Process gse_agent.exe start failed"
        call :multi_report_step_status
        exit /b 2
    ) else (
...
:is_process_stop_ok
    ...
    if %errorlevel% neq 0 (
        ...
        call :print FAILED setup_agent DONE "Process gse_agent_daemon.exe stop failed"
        call :multi_report_step_status
        exit /b 3
    )
    ...
    if %errorlevel% neq 0 (
        ...
        call :print FAILED setup_agent DONE "Process gse_agent.exe stop failed"
        call :multi_report_step_status
        exit /b 3
    )
...
:is_connected
    ...
    for %%a in (%CLUSTER_PORT%,%DATA_PORT%) do (
        ...
        if !errorlevel! equ 0 (
            ...
            call :print INFO check_deploy_result - "%%a is not connect to gse server , failed"
            call :multi_report_step_status
        )
    )
goto :EOF
...
:check_polices_pagent_to_upstream
        ...
        for /f %%i in ('%TMP_DIR%\tcping.exe -i 0.01 %%p %CLUSTER_PORT% ^| findstr successful') do (
            ...
            rem echo %%s
                if %%s GEQ 2 (
                    ...
                    call :print INFO check_env - "connect to upstream cluster server: %%p %%a failed"
    ...
    if not defined "%network_not_reachable%" (
        call :print FAILED check_env DONE "%network_not_reachable% is not reachable"    
        call :multi_report_step_status
        exit /b 1
    )
        ...
        for /f %%i in ('%TMP_DIR%\tcping.exe -i 0.01 %%p %DATA_PORT% ^| findstr successful') do (
            ...
            rem echo %%s
                if %%s GEQ 2 (
                    ...
                    call :print INFO check_env - "connect to upstream data server: %%p %%a failed"
                    set data_network_not_reachable="%data_network_not_reachable% %%p to %%a\n"
                    call :multi_report_step_status
                )
            )
        )
    ...
    if not defined "%data_network_not_reachable%" (
        call :print FAILED check_env DONE "%data_network_not_reachable% is not reachable"    
        call :multi_report_step_status
        exit /b 1
    )
    ...
    call :multi_report_step_status
    echo=
    set file_data_network_not_reachable=
            ...
            for /f %%i in ('%TMP_DIR%\tcping.exe -i 0.01 %%p %%a ^| findstr successful') do (
                ...
                rem echo %%s
                    if %%s GEQ 2 (
                        ...
                        call :print INFO check_env - "connect to upstream file server: %%p %%a failed"
    ...
    if not defined "%file_network_not_reachable%" (
        call :print FAILED check_env DONE "%file_network_not_reachable% is not reachable"    
        call :multi_report_step_status
        exit /b 1
    )
...
:check_polices_agent_to_upstream
        ...
        for /f %%i in ('%TMP_DIR%\tcping.exe -i 0.01 %%p %CLUSTER_PORT% ^| findstr successful') do (
            ...
            rem echo %%s
                if %%s GEQ 2 (
                    ...
                    call :print WARN check_env - "gse server %%p %%a can not reachable"
                    set network_not_reachable="%network_not_reachable% %%p to %%a \n"
                    call :multi_report_step_status
                )
            )
    ...
    call :print INFO check_env - "check if it is reachable to port %FILE_SVR_PORT%,%BT_PORT%-%TRACKER_PORT% of %FILE_SERVER_IP% GSE_BTFILE_SERVER"
    call :multi_report_step_status
    set network_not_reachable=
    for %%p in (%FILE_SERVER_IP%) do (
        rem for %%a in (28668,28625,28925,10020) do (
...
:uninstall_agent_service
    ...
    if %errorlevel% equ 0 (
        ...
        call :print FAILED uninstall_agent_service DONE "service gse_agent_daemon_%service_id% remove failed"
        call :multi_report_step_status
        exit /b 1
    )
...
:remove_agent_bin
    ...
    if not exist %GSE_AGENT_BIN_DIR% (
        ...
        if not exist %GSE_AGENT_BIN_DIR% (
            ...
            call :print FAILED setup_agent DONE "Directory %GSE_AGENT_BIN_DIR% removed, agent\bin dir removed failed"
            call :multi_report_step_status
            exit /b 1
        )
    )
    ...
    for %%p in (gse_agent.conf) do (
        ...
        for /f %%i in (%TMP_DIR%\nm.test.HHHHHHH) do (
            if %%i equ 200 (
                ...
                call :print FAILED get_config DONE "request config %%p failed. request info:%CLOUD_ID%,%LAN_ETH_IP%,%node_type%,%%p,%TOKEN%. http status:%%i"
                call :multi_report_step_status
                exit /b 1
            )
        )
...
:setup_agent
    ...
    if %errorlevel% EQU 0 (
        ...
        call :print FAILED setup_agent DONE "setup agent , copy cert dir failed"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    if %errorlevel% EQU 0 (
        ...
        call :print FAILED setup_agent DONE "setup agent , copy bin dir failed"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    if exist %TMP_DIR%\gse_agent.conf (
        ...
        call :print FAILED setup_agent DONE "get config file gse_agent.conf failed"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    if %errorlevel% EQU 0 (
        ...
        call :print FAILED setup_agent DONE "setup agent , config file gse_agent.conf is error"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    for %%p in (gse_agent.conf) do (
        if exist %TMP_DIR%\%%p (
            ...
            if not exist %GSE_AGENT_ETC_DIR%\%%p (
                call :print FAILED setup_agent DONE "setup agent , copy config file to %AGENT_SETUP_PATH%\agent\etc\ failed"
                call :multi_report_step_status
                exit /b 1
            )
        ) else (
            call :print FAILED setup_agent DONE "%%p file download failed. please check."
            call :multi_report_step_status
            exit /b 1
        )
    )
...
:stop_basic_gse_plugin
    ...
    if exist %AGENT_SETUP_PATH%\plugins\bin (
        ...
        if %errorlevel% neq 0 (
            call :print FAILED stop_plugin DONE "Process basereport.exe stop failed"
            call :multi_report_step_status
            exit /b 1
        ) else (
        ...
        if %errorlevel% neq 0 (
            call :print FAILED stop_plugin DONE "Process processbeat.exe stop failed"
            call :multi_report_step_status
            exit /b 1
        ) else (
...
:download_pkg
    ...
    for %%p in (%PKG_NAME%) do (
        ...
        if %http_code_res% EQU 200 (
            ...
            call :print FAILED download_pkg DONE "file %PKG_NAME% download failed. url:%DOWNLOAD_URL%/agent/windows/%CPU_ARCH%/%PKG_NAME%, http_status:%http_code_res%"
            call :multi_report_step_status
            exit /b 1
        )
goto :EOF
...
:check_pkgtool
    if exist %TMP_DIR%\curl.exe (
        ...
        call :print FAILED check_env DONE "%TMP_DIR%\curl.exe not found"
        call :multi_report_step_status
        exit /b 1
    )
goto :EOF
    ...
    for %%p in (unixdate.exe,jq.exe,7z.dll,7z.exe,handle.exe,tcping.exe,ntrights.exe) do (
        ...
        for /f %%i in (%TMP_DIR%\nm.test.EEEEEEE) do (
            if %%i equ 200 (
                ...
                call :print FAILED download_exe DONE "file %%p download failed, url:%DOWNLOAD_URL%/%%p, http_status:%%i"
                call :multi_report_step_status
                exit /b 1
            )
...
:check_disk_space
    for /f "tokens=1,2 delims=\" %%s in ("%AGENT_SETUP_PATH%") do (
        for /f %%i in ('wmic LogicalDisk where "Caption='%%s'" get FreeSpace ^| findstr "[0-9]"') do (
            if %%i LSS 307200 (
                call :print FAILED check_env DONE "no enough space left on %TMP_DIR%"
                call :multi_report_step_status
                exit /b 1
            ) else (
...
:check_dir_permission
    ...
    if %errorlevel% equ 0 (
        ...
        call :print FAILED check_env DONE "create temp files failed in %TMP_DIR%"
        call :multi_report_step_status
        exit /b 1
    )
goto :EOF
...
:check_download_url
    for /f "delims=" %%i in ('%TMP_DIR%\curl.exe --silent %DOWNLOAD_URL%/%PKG_NAME% -Iw "%%{http_code}"') do (set http_status=%%i)
        if "%http_status%" == "200" (
            ...
            call :print FAILED check_env DONE "check resource %DOWNLOAD_URL%/%PKG_NAME% url failed, http_status:%http_status%"
            call :multi_report_step_status
            exit /b 1
        )
    )
...
:check_env
    ...
    if "%NODE_TYPE%" == "agent" (
        ...
        call :print FAILED check_env DONE "NODE_TYPE exist , but it's not agent or pagent , plz check"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    if %errorlevel% equ 0 (
        ...
        call :print FAILED check_env DONE "checking prerequisite done, result: SUCCESS"
        call :multi_report_step_status
        exit /b 1
    )
goto :EOF
...
:remove_agent_pro
    ...
    if "%_service_id%" == "" (
        call :print FAILED check_deploy_result DONE "service_id -%_service_id%- get failed failed"
    ...
    if not exist %PURE_AGENT_SETNUP_PATH% (
        ...
        if not exist %PURE_AGENT_SETNUP_PATH% (
            ...
            call :print FAILED remove_agent DONE "agent removed failed"
            rem if exist %tmp_json_resp_report_log% (call :last_log_process)
            call :multi_report_step_status
...
:check_uninstall_result
    ...
    if %service% EQU 0 (
        if %dirs% EQU 0 (
            ...
            call :print FAILED remove_agent DONE "gse agent has been uninstalled failed"
            call :multi_report_step_status
            call :last_log_process
            if exist %tmp_json_resp_report_log% (call :last_log_process)
        )
...
:register_agent_id
    if NOT EXIST %gse_winagent_home%\\agent\\bin\\gse_agent.exe (
        call :print FAILED register_agent_id DONE "gse_agent.exe not exists"
        call :multi_report_step_status
        exit /b 1
    )
...
:start_register_agent_id
    ...
    if %errorlevel% EQU 0 (
        ...
        call :print FAILED start_register_agent_id DONE "register agent id failed"
        call :multi_report_step_status
        exit /b 1
    )
goto :EOF
...
:re_register_agent_id
    ...
    if %errorlevel% EQU 0 (
        ...
        call :print FAILED re_register_agent_id DONE "re_register agent id failed"
        call :multi_report_step_status
        exit /b 1
    )
goto :EOF
...
:report_gse_healthz
    for /F "tokens=*" %%i in ('%GSE_AGENT_BIN_DIR%\gse_agent.exe --healthz -f %GSE_AGENT_ETC_DIR%\gse_agent.conf') do (set healthzinfo=%%i)
    echo call :print INFO report_healthz DONE "!healthzinfo!"
goto :EOF
...
:check_gse_healthz
    ...
    if "%base%" == "ok" (
        ...
        call :print FAILED check_gse_healthz DONE "data.base is unhealth: %base%"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    if "%cluster%" == "ok" (
        ...
        call :print FAILED check_gse_healthz DONE "data.cluster is unhealth: %cluster%"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    if "%data%" == "ok" (
        ...
        call :print FAILED check_gse_healthz DONE "data.data is unhealth: %data%"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    if "%file%" == "ok" (
        ...
        call :print FAILED check_gse_healthz DONE "data.file is unhealth: %file%"
        call :multi_report_step_status
        exit /b 1
    )
...
:unregister_agent_id
    if NOT EXIST %gse_winagent_home%\\agent\\bin\\gse_agent.exe (
        call :print FAILED unregister_agent_id DONE "%gse_winagent_home%\\agent\\bin\\gse_agent.exe not exists"
        call :multi_report_step_status
        exit /b 1
    )
    ...
    if %errorlevel% EQU 0 (
        ...
        call :print FAILED register_agent_id DONE "unregister agent id failed"
        call :multi_report_step_status
        exit /b 1
    )
goto :EOF
...
:install_gse_agent_service_with_user_special
    if "%INSTALL_USER%" == "" (
        ...
        if !errorlevel! equ 0 (
            ...
            call :print FAILED setup_agent DONE "create gse_agent service without special user failed"
            call :multi_report_step_status
            exit /b 1
        )
    ) else (
        ...
        if !errorlevel! equ 0 (
            ...
            call :print FAILED setup_agent DONE "create gse_agent service with special user %INSTALL_USER% failed"
            call :multi_report_step_status
            exit /b 1
        )
    )
    ...
    if %errorlevel% NEQ 0 (
        ...
        call :print FAILED setup_agent DONE "start service gse_agent_daemon_%service_id% failed, with error info: !res!"
        call :multi_report_step_status
        exit /b 1
    ) else (
        call :print INFO setup_agent - "start service gse_agent_daemon_%service_id% succeed"
