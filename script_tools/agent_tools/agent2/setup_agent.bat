@echo off
rem gse agent 2.0 scripts, for bk_nodeman 2.0
...
set PKG_NAME=
set CLOUD_ID=
set JOINT_DEBUG_SWITCH=TRUE
set GSE_AGENT_CONFIG="gse_agent.conf"
set AGENT_CLEAN_UP_DIR=bin

:CheckOpts
if "%1" EQU "-h" goto help
if "%1" EQU "-R" goto remove_agent_pro
if "%1" EQU "-n" (set NAME=%~2) && shift && shift && goto CheckOpts
if "%1" EQU "-t" (set VERSION=%~2) && shift && shift && goto CheckOpts
...
set PKG_NAME=%NAME%-%VERSION%.tgz
if "%PROCESSOR_ARCHITECTURE%" == "x86" (set GSE_AGENT_CONFIG_PATH=%DOWNLOAD_URL%/agent/linux/x86) else (set PGSE_AGENT_CONFIG_PATH=%DOWNLOAD_URL%/agent/linux/x86_64)
...
:download_pkg
    call :print INFO download_pkg - "download gse agent package from %GSE_AGENT_CONFIG_PATH%/%PKG_NAME% START"
    call :multi_report_step_status

    cd %TMP_DIR% && DEL /F /S /Q %PKG_NAME% 1>nul 2>&1

    for %%p in (%PKG_NAME%) do (
        for /F %%i in ('curl --connect-timeout 5 -o %TMP_DIR%/%%p --progress-bar -w "%%{http_code}" %GSE_AGENT_CONFIG_PATH%/%%p') do (set http_status=%%i)
        if "%http_status%" == "200" (
            call :print INFO download_pkg - "gse_agent package %GSE_AGENT_CONFIG_PATH%/%%p download succeeded"
            call :print INFO report_cpu_arch DONE "%CPU_ARCH%"
        ) else (
            call :print FAIL download_pkg FAILED "file %%p download failed. url:%GSE_AGENT_CONFIG_PATH%/%%p, http_status:%http_status%"
        )
    )
goto :EOF
...
:install_gse_agent_service_with_user_special
    if "%INSTALL_USER%" == "" (
        %gse_winagent_home%\\agent\\bin\\gse_agent_daemon.exe --install -f %gse_winagent_home%\\agent\\etc\\%GSE_AGENT_CONFIG% --name gse_agent_daemon_%service_id%
        if %errorlevel% equ 0 (
        ...
        %gse_winagent_home%\\agent\\bin\\gse_agent_daemon.exe --install -f %gse_winagent_home%\\agent\\etc\\%GSE_AGENT_CONFIG% --name gse_agent_daemon_%service_id% --user .\\%INSTALL_USER% --pwd %INSTALL_PASSWORD% --encode
        if %errorlevel% equ 0 (
    ...
    %gse_winagent_home%\\agent\\bin\\gse_agent_daemon.exe --start --name gse_agent_daemon_%service_id% -f %gse_winagent_home%\\agent\\etc\\%GSE_AGENT_CONFIG%
goto :EOF

:check_healthz_by_gse 
    FOR /F "delims=" %i IN ('%gse_winagent_home%\\agent\\bin\\gse_agent.exe --healthz -f %gse_winagent_home%\\agent\\etc\\%GSE_AGENT_CONFIG%') DO set healthz_result=%i
    if "%errorlevel%" equ 0 (
        call :print INFO setup_agent DONE "gse_agent healthz check succeed, check result: %healthz_result%"
        if exist %tmp_json_resp_report_log% (call :last_log_process)
        if %errorlevel% equ 0 (goto :EOF)
    ) else (
        call :print FAIL setup_agent DONE "gse_agent healthz check failed"
        if exist %tmp_json_resp_report_log% (call :last_log_process)
        if %errorlevel% equ 1 (goto :EOF)
    )
    
:quit_legacy_agent
    rem 兼容性处睆，坜用并删除耝版本agent
