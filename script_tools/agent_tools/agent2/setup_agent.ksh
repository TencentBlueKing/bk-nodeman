...
AGENT_CONFIG_FILE=gse_agent.conf
...
divide_version () {
    ...
    PKG_NAME=gse_client-aix"${OS_VERSION}"-powerpc-"${VERSION}".tgz
    COMPLETE_DOWNLOAD_URL="${DOWNLOAD_URL}"/agent/aix/powerpc/"${PKG_NAME}"
}
...
pre_view () {
   ...
   if [[ -f $AGENT_SETUP_PATH/etc/$AGENT_CONFIG_FILE ]]; then
...
get_config () {
    ...
    set -A config $AGENT_CONFIG_FILE
...
download_pkg () {
    ...
    log download_pkg START "download gse agent package from $COMPLETE_DOWNLOAD_URL."
    cd "$TMP_DIR" && rm -f "$PKG_NAME" "$AGENT_CONFIG_FILE.$LAN_ETH_IP"

    http_status=$(http_proxy=$HTTP_PROXY https_proxy=$HTTPS_PROXY curl -O $COMPLETE_DOWNLOAD_URL \
            --silent -w "%{http_code}")
    # HTTP status 000需要进一步研究
    if [[ $http_status != "200" ]] && [[ "$http_status" != "000" ]]; then
        fail download_pkg FAILED "file $f download failed. (url:$COMPLETE_DOWNLOAD_URL, http_status:$http_status)"
    fi
...
check_heathz_by_gse () {
    local result
    result=$("${AGENT_SETUP_PATH}"/bin/gse_agent --heathz)
    execution_code=$($?)
    if ! [ "${execution_code}" -eq 0 ]; then
        fail check_env FAILED "gse heathz check failed. execution_code: ${execution_code}, result: ${result}"
    fi
}
