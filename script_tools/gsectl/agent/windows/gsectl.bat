...
set gse_agent_config_path=%gse_winagent_home:"=%\\agent\\etc\\gse_agent.conf
...
rem 启动进程
gse_agent_daemon.exe --install -f %gse_agent_config_path% --name gse_agent_daemon%_service_id% 1>nul 2>&1
gse_agent_daemon.exe --start --name gse_agent_daemon%_service_id% -f %gse_agent_config_path% 1>nul 2>&1
...
if !errorlevel! neq 0 (
    ...
    rem 停止进程
    gse_agent_daemon.exe --quit -f %gse_agent_config_path% --name gse_agent_daemon%_service_id% 1>nul 2>&1
    ping -n 1 127.0.0.1 >nul 2>&1
    wmic process where name="'gse_agent_daemon.exe' and ExecutablePath='%gse_winagent_home:"=%\\agent\\bin\\gse_agent_daemon.exe'" call terminate 1>nul 2>&1
    wmic process where name="'gse_agent.exe' and ExecutablePath='%gse_winagent_home:"=%\\agent\\bin\\gse_agent.exe'" call terminate 1>nul 2>&1
    wmic process where name="'basereport.exe' and ExecutablePath='%gse_winagent_home:"=%\\plugins\\bin\\basereport.exe'" call terminate 1>nul 2>&1
    ...
    if !errorlevel! neq 0 (
        wmic process where name='gse_agent.exe' get processid,executablepath,name 2>&1 | findstr /i "%gse_agent_path%" 1>nul 2>&1
        if !errorlevel! neq 0 (
            echo [%cu_date% %cu_time%] stop done -- stop done/stop process >nul
            exit /b 0
            goto :EOF
        )
    ) else (
        echo [%cu_date% %cu_time%] stop fail -- gse_agent stop fail/stop process >nul
        exit /b 1
    )
)
goto :EOF
