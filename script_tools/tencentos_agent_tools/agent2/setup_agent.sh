...
is_port_connected_by_pid() {
    ...
    if lsof -nPi tcp:"$port" -sTCP:ESTABLISHED | grep -q "^.*$pid.*"; then
        return 0
    else
        return 1
    fi
...
setup_startup_scripts () {
    ...
    # 目前的gsectl仍然包含ps和awk指令，等gsectl完全适配更新之后可删除下面两行sed替换命令
    sed -i -e 's\%P\ppid\g' -e 's\|%p\pid\g' -e 's\|%a\args\g' $AGENT_SETUP_PATH/bin/gsectl
    sed -i 's/awk -F'\''|'\'' '\''$1 == 1 \&\& $3 ~ \/gse_agent\/'\'' | awk -F'\''|'\'' '\''{print $2}'\''/awk '\''$1 == 1 \&\& $3 ~ \/gse_agent\/ {print $2}'\''/g' $AGENT_SETUP_PATH/bin/gsectl
...
start_agent () {
    ...
    # 目前的gsectl仍然包含ps和awk指令，等gsectl完全适配更新之后可删除下面两行sed替换命令
    sed -i -e 's/|%p/pid/g' -e 's/%P/ppid/g' -e 's/|%a/args/g' "$AGENT_SETUP_PATH"/bin/gsectl
    sed -i 's/awk -F'\''|'\'' '\''$1 == 1 \&\& $3 ~ \/gse_agent\/'\'' | awk -F'\''|'\'' '\''{print $2}'\''/awk '\''$1 == 1 \&\& $3 ~ \/gse_agent\/ {print $2}'\''/g' "$AGENT_SETUP_PATH"/bin/gsectl start
